...
import { buildBoletaPDF } from '../pdf/pdfBuilder.js';
import { sendBoletaEmail } from '../mail/mailer.js';
import {
...
router.post('/boletas/:dpi/firmar', async (req, res) => {
    try {
        ...
        const sigData = String(req.body.sig_data || '');
        ...
        const row = await getSueldoFullRow(sueldoId);
        const m = monthFromVal(row.fecha_pago);
        const showBono14 = m === 7, showAguinaldo = m === 12;

        const percepciones =
            Number(row.salario_quincenal || 0) + Number(row.bonificacion || 0) +
            (showBono14 ? Number(row.bono_14 || 0) : 0) + (showAguinaldo ? Number(row.aguinaldo || 0) : 0) +
            Number(row.horas_extra || 0) + Number(row.comisiones || 0);

        const deducciones =
            Number(row.igss || 0) + Number(row.isr || 0) + Number(row.descuentos_dasa || 0) + Number(row.seguro_vida || 0);

        const pdf = await buildBoletaPDF({
            empresa: 'Drogueria Americana, S.A',
            empleado: { nombre: row.nombre, cod_empleado: row.cod_empleado, dpi: row.dpi, puesto: row.puesto },
            periodo: { fecha_inicio: fmtDate(row.fecha_inicio), fecha_fin: fmtDate(row.fecha_fin) },
            sueldo: {
                salario_quincenal: row.salario_quincenal, bonificacion: row.bonificacion,
                igss: row.igss, isr: row.isr, descuentos_dasa: row.descuentos_dasa, seguro_vida: row.seguro_vida,
                fecha_pago: fmtDate(row.fecha_pago)
            },
            totales: { percepciones, deducciones, neto: percepciones - deducciones },
            firmaDataURL: sigData
        });

        await sendBoletaEmail({
            subject: `${process.env.MAIL_SUBJECT_PREFIX || 'Boleta firmada'} - ${row.etiqueta}`,
            to: process.env.MAIL_TO,
            pdfBuffer: pdf,
            filename: `boleta-${dpi}-${row.etiqueta.replace(/\s+/g, '_')}.pdf`
        });
